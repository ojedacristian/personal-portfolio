...
export const PortfolioCard: React.FC<PortfolioCardProps> = ({ title, desc, github, img, live }) => {
  return (
        <figure className="group relative mt-6 overflow-hidden rounded-2xl bg-brand-orange text-white hover:cursor-pointer">
            <img src={ img } alt={ title } className='min-w-full group-hover:opacity-60' />
            ...
            <p className='invisible absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-6 font-lato font-bold  opacity-0 transition-all duration-500 ease-in-out group-hover:visible group-hover:translate-y-0 group-hover:opacity-100'>
                { desc }
                </p>
